...
class StockActor(symbol: String) extends Actor {
  ...
  var globalUtility = 0.0;
  
  protected[this] var watchers: HashSet[ActorRef] = HashSet.empty[ActorRef]

  // A random data set which uses stockQuote.newPrice to get each data point
  var stockHistory: Queue[java.lang.Double] = {
    lazy val initialPrices: Stream[java.lang.Double] = (new Random().nextDouble * 1) #:: initialPrices.map(previous => stockQuote.newPrice(previous))
  ...
  def receive = {
    case FetchLatest =>
      ...
      // notify watchers
      watchers.foreach(_ ! StockUpdate("Global", globalUtility))
    //  watchers.foreach(_ ! GlobalUtilityUpdate(globalUtility))
...
class StocksActor extends Actor {
  ...
  def receive = {
    ...
    case utilityUpdate @ UtilityUpdate(utility) =>
      context.children.foreach(_.forward(utilityUpdate))
  }
}
...
case class GlobalUtilityUpdate(utility : Double);

case class UtilityUpdate(utility : Double);
