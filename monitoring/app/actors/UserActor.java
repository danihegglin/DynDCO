...
public class UserActor extends UntypedActor {
    ...
    public UserActor(WebSocket.Out<JsonNode> out) {
       ...
       //for (String stockSymbol : defaultStocks) {
        ...
        StocksActor.stocksActor().tell(new WatchStock("Global"), getSelf());
    }
    
    public void onReceive(Object message) {
    	
    	if(message instanceof GlobalUtilityUpdate){
    		
    		// push the utility update to the client
    		GlobalUtilityUpdate globalUtilityUpdate = (GlobalUtilityUpdate) message;
    		ObjectNode utilityUpdateMessage = Json.newObject();
    		utilityUpdateMessage.put("type", "stockupdate");
    		utilityUpdateMessage.put("symbol", "Global"); // FIXME make dynamic
    		utilityUpdateMessage.put("price", "2");
    		
	        out.write(utilityUpdateMessage);
    	}
